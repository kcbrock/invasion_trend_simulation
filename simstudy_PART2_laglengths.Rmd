...
```
look for the elbow (4-5 is good here)
...
```
okay, now run it for a specific penalty level, to get the desired change point #
...
sum(segwmeans[1:(startptindex-1)]) / chpoints[startptindex]
sqrt(sum(segwvars[1:(startptindex-1)]) / chpoints[startptindex])

print("truncated seg length, mean, std:")
chpoints[endptindex] - chpoints[startptindex]
sum(segwmeans[startptindex:(endptindex -1)]) / (chpoints[endptindex] - chpoints[startptindex])
sqrt(sum(segwvars[startptindex:(endptindex -1)]) / (chpoints[endptindex] - chpoints[startptindex]))

print("ending seg length, mean, std:")
nrow(accumulated_A) - chpoints[endptindex]
sum(segwmeans[endptindex:length(segwmeans)]) / (chpoints[length(chpoints)]- chpoints[endptindex])
sqrt(sum(segwvars[endptindex:length(segwvars)]) / (chpoints[length(chpoints)]- chpoints[endptindex]))

```
```{r}
sum(segwmeans[startptindex:(endptindex -1)])
(chpoints[startptindex]- chpoints[endptindex])
```
...
#### Changepoint - Report Date
```{r}
plot(accumulated_B$Taxa_Reported)
```

```{r}
b1 <- changepoint::cpt.meanvar(accumulated_B$Taxa_Reported, penalty = "CROPS",  pen.value=c(5,500), method = "PELT", Q = 25, test.stat = "Normal", class = TRUE, param.estimates = TRUE, minseglen = 5)
```

```{r}
plot(b1, diagnostic = TRUE)
```
look for the elbow (4-5 is good here)
...
selectedcp = 3
#changepoint::plot(a1, cpt.width = 4, col = "black", cpt.col = "blue", ylab = "# of Taxa Vouchered", xlab = "Years (since beginning of time window)")
changepoint::plot(b1, ncpts = selectedcp)
```

```{r}
changepoint::cpts.full(b1) + (as.numeric(accumulated_B$Year[[1]]) - 1)
```
okay, now run it for a specific penalty level, to get the desired change point #
...
firstrow = 4
secondrow =  firstrow + 1
b2 <- changepoint::cpt.meanvar(accumulated_B$Taxa_Reported, penalty = "Manual",  pen.value=((pen.value.full(b1)[firstrow] + pen.value.full(b1)[secondrow]) /2) , method = "PELT", Q = 10, test.stat = "Normal", class = TRUE, param.estimates = TRUE, minseglen = 5)
```
...
changepoint::cpts(b2) + (as.numeric(accumulated_B$Year[[1]]) - 1)

changepoint::param.est(b2)
```
...
startseg <- subset(accumulated_B, Year <= startsegbefore)
truncated <- subset(accumulated_B, Year > startsegbefore & Year < endsegafter)
endseg <- subset(accumulated_B, Year >= endsegafter)

print("start seg length, mean, std: ")
nrow(startseg)
round(mean(startseg$Taxa_Reported), 2)
round(stdev(startseg$Taxa_Reported), 2)
print("truncated seg length, mean, std:")
nrow(truncated)
round(mean(truncated$Taxa_Reported), 2)
round(stdev(truncated$Taxa_Reported), 2)
print("ending seg length, mean, std:")
nrow(endseg)
round(mean(endseg$Taxa_Reported), 2)
round(stdev(endseg$Taxa_Reported), 2)
```
...
changepoint::cpts(c2) + (as.numeric(accumulated_C$Year[[1]]) - 1)

changepoint::param.est(c2)
```
```{r}
startsegbefore = as.numeric(accumulated_C$Year[1]) -1
endsegafter = 1964

startseg <- subset(accumulated_C, Year <= startsegbefore)
truncated <- subset(accumulated_C, Year > startsegbefore & Year < endsegafter)
endseg <- subset(accumulated_C, Year >= endsegafter)

print("start seg length, mean, std: ")
nrow(startseg)
round(mean(startseg$Taxa_Introduced), 2)
round(stdev(startseg$Taxa_Introduced), 2)
print("truncated seg length, mean, std:")
nrow(truncated)
round(mean(truncated$Taxa_Introduced), 2)
round(stdev(truncated$Taxa_Introduced), 2)
print("ending seg length, mean, std:")
nrow(endseg)
round(mean(endseg$Taxa_Introduced), 2)
round(stdev(endseg$Taxa_Introduced), 2)
```
```{r}
accumulated_C$Year[1]
```
...
ggplot(DATA, aes(x=as.numeric(Year), y=as.numeric(accumulated_species), group = Analysis_Type, color = Analysis_Type)) +
 ...
 theme_classic() +
  ...
  labs(title = "All Lags Equal and Unchanging", x = "Year", y = "Number of Species")
ggsave(filename = "ConstantLagScenario.png", device = "png", scale = 1, width =6, dpi = 600)
```
...
# steady decline
allthemeans <- as.data.frame(seq(from = 10, to = 0.083, length.out = nrow(alltheyears))) 

#or exponential decline

#allthemeans <- read.csv("expdecline.csv",   header=F, sep=',', na.strings=c("","NA"), stringsAsFactors=F)
...
Report_Compile_Lag <- mapply(x = paramdf$xi, o =paramdf$omega, a = paramdf$alpha, FUN = get_lag)
 #dists <- cbind(alltheyears, as.data.frame(t(dists)))
...
sim_spp2$All_Processing_Lags <- Intro_Nat_Lag$Intro_Nat_Lag + Nat_Detect_Lag$Nat_Detect_Lag + Detect_Vouch_Lag$Detect_Vouch_Lag + Vouch_ID_Lag$Vouch_ID_Lag + ID_Report_Lag$ID_Report_Lag
#reordering according to Report Year
...
selectedcp = 3
#changepoint::plot(a1, cpt.width = 4, col = "black", cpt.col = "blue", ylab = "# of Taxa Vouchered", xlab = "Years (since beginning of time window)")
changepoint::plot(a1, ncpts = selectedcp, type = "p")
```
...
changepoint::cpts(a2) + (accumulated_A$Year[1] -1)
...
startseg <- subset(accumulated_A, Year <= startsegbefore)
truncated <- subset(accumulated_A, Year > startsegbefore & Year < endsegafter)
endseg <- subset(accumulated_A, Year >= endsegafter)

print("start seg length, mean, std: ")
nrow(startseg)
round(mean(startseg$Taxa_Vouchered), 2)
round(stdev(startseg$Taxa_Vouchered), 2)
print("truncated seg length, mean, std:")
nrow(truncated)
round(mean(truncated$Taxa_Vouchered), 2)
round(stdev(truncated$Taxa_Vouchered), 2)
print("ending seg length, mean, std:")
nrow(endseg)
round(mean(endseg$Taxa_Vouchered), 2)
round(stdev(endseg$Taxa_Vouchered), 2)
```
...
ggplot(DATA, aes(x=as.numeric(Year), y=as.numeric(accumulated_species), group = Analysis_Type, color = Analysis_Type)) +
  ...
  scale_x_continuous(breaks = c(1900, 1920, 1940, 1960, 1980, 2000, 2020)) +
   guides(colour = guide_legend(reverse=T)) +
 theme_classic() +
  labs(title = "Data Processing Lags Steadily Become Shorter", x = "Year", y = "Number of Species")
ggsave(filename = "DPlagsshorten.png", device = "png", scale = 1, width =6, dpi = 600)
```
...
mean <- changepoint::param.est(a2)$mean
sqrt <- sqrt(changepoint::param.est(a2)$variance)
...
nrow(endseg)
round(mean(endseg$Taxa_Vouchered), 2)
round(mean(sqrt[(endptindex +1):length(sqrt)]), 2)
```
```{r}
sqrt[(endptindex +1):length(sqrt)]
...
ggplot(DATA, aes(x=as.numeric(Year), y=as.numeric(accumulated_species), group = Analysis_Type, color = Analysis_Type)) +
  ...
  scale_x_continuous(breaks = c(1920, 1940, 1960, 1980, 2000, 2020)) +
   guides(colour = guide_legend(reverse=T)) +
 theme_classic() +
  labs(title = "Field Surveying Lags Steadily Become Shorter", x = "Year", y = "Number of Species")
ggsave(filename = "SteadyShortenFieldLags.png", device = "png", scale = 1, width =6, dpi = 600)
```
...
sim_spp4$Report_Year <- sim_spp4$Naturalization_Year + Nat_Detect_Lag$Nat_Detect_Lag + Detect_Vouch_Lag$Detect_Vouch_Lag + Vouch_ID_Lag$Vouch_ID_Lag + ID_Report_Lag$ID_Report_Lag 


sim_spp4$Compile_Year <- sim_spp4$Naturalization_Year + Nat_Detect_Lag$Nat_Detect_Lag + Detect_Vouch_Lag$Detect_Vouch_Lag + Vouch_ID_Lag$Vouch_ID_Lag + ID_Report_Lag$ID_Report_Lag + Report_Compile_Lag$Report_Compile_Lag
...
ggplot(DATA, aes(x=as.numeric(Year), y=as.numeric(accumulated_species), group = Analysis_Type, color = Analysis_Type)) +
  ...
  scale_x_continuous(breaks = c(1920, 1940, 1960, 1980, 2000, 2020)) +
 theme_classic() +
  labs(title = "Field Surveying & Data Processing Lags Become Shorter", x = "Year", y = "Number of Species")
ggsave(filename = "FSandDP_steadyshorten.png", device = "png", scale = 1, width =6, dpi = 600)
```
