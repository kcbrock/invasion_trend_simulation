...
### Changepoint mess around


```{r}
fit_bp = strucchange::breakpoints(accumulated$Taxa_Naturalized ~ 1)
summary(fit_bp)
```
```{r}
fit_bp$breakpoints + 1920
```
...
#   geom_point() +
#   geom_vline(xintercept=(fit_bp$breakpoints + 1920)) +
#   theme_classic() +
...
#### Nat Rate of the linear segment
...
round((enddate_num - startdate_num) / (enddate - startdate), digits =0)
```
```{r}
...
From tutorial by Rebecca Killick
...
```{r}
pen.value.full(a1)[[selectedcp]]
```
```{r}
((pen.value.full(a1)[2] + pen.value.full(a1)[3]) /2)
```
...
```{r}
a2 <- changepoint::cpt.meanvar(accumulated_A$Taxa_Vouchered, penalty = "Manual",  pen.value=((pen.value.full(a1)[2] + pen.value.full(a1)[3]) /2) , method = "PELT", Q = 10, test.stat = "Normal", class = TRUE, param.estimates = TRUE, minseglen = 5)
a2
```
...
so, the changepoint analysis should be able to calculate the rate of the linear sergment too - that's what those red lines are for.
...
#How many species per year, on average? 
#Should be about ten (1000 simulated species over 100 years)
startdate = fit_bp$breakpoints[1] +1920
enddate = fit_bp$breakpoints[2] +1920
startdate_num <- accumulated[which(accumulated$Year == startdate), 3]
enddate_num <- accumulated[which(accumulated$Year == enddate), 3]
...
```{r}
fit_bp = strucchange::breakpoints(accumulated_C$Taxa_Introduced ~ 1)
summary(fit_bp)
```
...
#### Breakpoints
```{r}
fit_bp$breakpoints + as.numeric(min(accumulated_C$Year))
```


```{r}
plot(Taxa_Introduced ~  Year, data = accumulated_C)
```
...
fit1 <- lm(Taxa_Introduced ~  Year, data = accumulated_C)
ggplotRegression(fit1)
```


#### Length of level beginning
```{r}
(fit_bp$breakpoints[1] + 1920) - 1920
```
#### Length of level end
```{r}
2020 - (fit_bp$breakpoints[3] + 1920)
```
...
allthemeans <- as.data.frame(seq(from = 35, to = 5, length.out = nrow(alltheyears))) 
