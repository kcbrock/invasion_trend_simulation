...
```
```{r}
knitr::opts_knit$set(root.dir = "C:/Users/Kelsey/Documents/rwork/simulation_study") #set the working directory
```
```{r}
...
###sim_spp2$Compile_Year <- sim_spp2$Naturalization_Year + Nat_Detect_Lag$Nat_Detect_Lag + Detect_Vouch_Lag$Detect_Vouch_Lag + Vouch_ID_Lag$Vouch_ID_Lag + ID_Report_Lag$ID_Report_Lag


sim_spp2$All_Processing_Lags <- Vouch_ID_Lag$Vouch_ID_Lag + ID_Report_Lag$ID_Report_Lag
#reordering according to Report Year
sim_spp2 <- sim_spp2 %>% dplyr::select(Species_IDNo, Intro_Year, Naturalization_Year, Detect_Year, Voucher_Year, ID_Year, Report_Year, All_Processing_Lags)
sim_spp2
```
...
###sim_spp3$Compile_Year <- sim_spp3$Naturalization_Year + Nat_Detect_Lag$Nat_Detect_Lag + Detect_Vouch_Lag$Detect_Vouch_Lag + Vouch_ID_Lag$Vouch_ID_Lag + ID_Report_Lag$ID_Report_Lag


sim_spp3$All_Processing_Lags <- Vouch_ID_Lag$Vouch_ID_Lag + ID_Report_Lag$ID_Report_Lag
#reordering according to Report Year
sim_spp3 <- sim_spp3 %>% dplyr::select(Species_IDNo, Intro_Year, Naturalization_Year, Detect_Year, Voucher_Year, ID_Year, Report_Year, All_Processing_Lags)
sim_spp3
```
...
#reordering according to Report Year
sim_spp4 <- sim_spp4 %>% dplyr::select(Species_IDNo, Intro_Year, Naturalization_Year, Detect_Year, Voucher_Year, ID_Year, Report_Year, All_Processing_Lags)
sim_spp4
```
...
# Scenario 5: Access Points Introduce different cutoff points : Including all specimens that mention naturalization
...
MAJOR DIFFERENCE: you only retrieve 60% of species, because not all labels mention the status of the plant



MAJOR DIFFERENCE: you get to crop based on the ID year, because you're looking at the vouchers themselves
```{r}
sim_spp5_crop <- subset(sim_spp5, Report_Year <= 2020)
sim_spp5_crop
```
...
ggplot(DATA, aes(x=as.numeric(Year), y=as.numeric(accumulated_species), group = Analysis_Type, color = Analysis_Type)) +
 ...
 theme_classic() +
  theme(legend.title=element_blank()) +
  labs(title = "60% Data Retrieval Rate", x = "Year", y = "Number of Species")
